...
async def reset_password(request: Request, db: Session = Depends(get_db)):
    ...
    if request.method == 'POST':
        ...
        if not user:
            ...
            context['success'] = 'SUCCESS! Instruction for reset password was sending to your email'
...
async def change_password(token: str, request: Request, db: Session = Depends(get_db)):
    ...
    if request.method == 'POST' and payload:
        ...
        if password and password != confirm_password:
            context['error'] = 'Пароли не совпадают или поля не заполнены'

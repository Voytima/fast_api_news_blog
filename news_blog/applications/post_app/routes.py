...
from applications.post_app.models import Post
from applications.post_app.forms import AddPostForm, UpdatePostForm
...
from ..auth_app.utils import get_user_by_token
...
async def show_post(uuid: str, request: Request, db: Session = Depends(get_db)):
    ...
    users = [db.query(User).filter(User.uid == comment.owner_uid).first().username
             for comment in comments]

    response_dict['comments'] = [{'username': username, 'content': comment.content} for
